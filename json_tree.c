...
#define _GNU_SOURCE 	/* pour le warning d'asprintf */
#define STRING_SIZE 128

char * constants[3] = {"false", "true", "null"};
static int idObject = 0;
static int idPair = 0;
static int idArray = 0;
static int idElem = 0;
 ...
 * \return char * : chaine représentant le JsonArray (a liberer ulterieurement)
 */
char * PrintDotJsonArray(const JsonArray * _array, int _id, int _idParent) {
    int i = 0;
    char * chaine = NULL;
    char * resultat = NULL;
    int currentId;

    /* 2 cas : 1)l'a est dans une paire et 2) le noeud a deja ete cree */
    if ( _id >= 0 ) {
        ...
        if ( resultat == NULL ) goto ERREUR_ALLOCATION;
        nbCar = sprintf(resultat,"array_%d [label=\"array\"];\n",currentId);
    ...
    /* pour chaque element de l'array, on complete la chaine resultat*/
    while ( i < _array->size ) {
        int nbCar;
        int tailleChaine;
        switch( _array->elements[i]->type ) {
            case string:
                /* creation du noeud elem_XX */
                idElem++;
                tailleChaine = STRING_SIZE+strlen(_array->elements[i]->value.string);
                chaine = (char*)malloc(tailleChaine*sizeof(char));
                if ( chaine == NULL ) goto ERREUR_ALLOCATION;
                nbCar = sprintf(chaine,"elem_%d [shape=box,label=\"%s\"];\n",idElem,_array->elements[i]->value.string);
                if ( nbCar >= tailleChaine ) goto ERREUR_MEMOIRE;
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
                /* creation de l'arc */
                sprintf(chaine,"array_%d -> elem_%d;\n",currentId,idElem);
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
                free(chaine);
            ...
            case integer:
                /* creation du noeud elem_XX */
                idElem++;
                tailleChaine = STRING_SIZE+32;
                chaine = (char*)malloc(tailleChaine*sizeof(char));
                if ( chaine == NULL ) goto ERREUR_ALLOCATION;
                nbCar = sprintf(chaine,"elem_%d [shape=box,label=\"%d\"];\n",idElem,_array->elements[i]->value.integer);
                if ( nbCar >= tailleChaine ) goto ERREUR_MEMOIRE;
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
            ...
            case real:
                ...
                chaine = (char*)malloc(tailleChaine*sizeof(char));
                if ( chaine == NULL ) goto ERREUR_ALLOCATION;
                nbCar = sprintf(chaine,"elem_%d [shape=box,label=\"%f\"];\n",idElem,_array->elements[i]->value.real);
                if ( nbCar >= tailleChaine ) goto ERREUR_MEMOIRE;
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
            ...
            case object:
                ...
                strcat(resultat,chaine);
                /* creation de l'arc */
                sprintf(chaine,"array_%d -> object_%d;\n",currentId,idObject);
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
                free(chaine);
                /* creation des sous-noeuds */
                chaine = PrintDotJsonObject(_array->elements[i]->value.object,idObject,currentId);
            ...
            case array:
                ...
                chaine = (char*)malloc(tailleChaine*sizeof(char));
                if ( chaine == NULL ) goto ERREUR_ALLOCATION;
                nbCar = sprintf(chaine,"array_%d [label=\"array\"];\n",idArray);
                if ( nbCar >= tailleChaine ) goto ERREUR_MEMOIRE;
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
                /* creation de l'arc */
                sprintf(chaine,"array_%d -> array_%d;\n",currentId,idArray);
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
                free(chaine);
                /* creation des sous-noeuds */
                chaine = PrintDotJsonArray(_array->elements[i]->value.array,idArray,currentId);
                if ( chaine == NULL ) goto ERREUR_CHAINE_VIDE;
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
            ...
            case constant:
                ...
                chaine = (char*)malloc(tailleChaine*sizeof(char));
                if ( chaine == NULL ) goto ERREUR_ALLOCATION;
                nbCar = sprintf(chaine,"elem_%d [shape=box,label=\"%s\"];\n",idElem,constants[_array->elements[i]->value.constant]);
                if ( nbCar >= tailleChaine ) goto ERREUR_MEMOIRE;
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
    ...
    while ( i < _object->size ) {
        ...
        switch( _object->members[i]->value->type ) {
            ...
            case array:
                ...
                chaine = (char*)malloc(tailleChaine*sizeof(char));
                if ( chaine == NULL ) goto ERREUR_ALLOCATION;
                nbCar = sprintf(chaine,"array_%d [label=\"array\\n%s\"];\n",idArray,_object->members[i]->string);
                if ( nbCar >= tailleChaine ) goto ERREUR_MEMOIRE;
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
                /* creation de l'arc */
                sprintf(chaine,"object_%d -> array_%d;\n",currentId,idArray);
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
                free(chaine);
                /* creation des sous-noeuds */
				chaine = PrintDotJsonArray(_object->members[i]->value->value.array,idArray,currentId);
                if ( chaine == NULL ) goto ERREUR_ALLOCATION;
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
				strcat(resultat,chaine);
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
            ...
            case constant:
                ...
                chaine = (char*)malloc(tailleChaine*sizeof(char));
                if ( chaine == NULL ) goto ERREUR_ALLOCATION;
                nbCar = sprintf(chaine,"pair_%d [shape=box,label=\"%s=%s\"];\n",idPair,_object->members[i]->string,constants[_object->members[i]->value->value.constant]);
                if ( nbCar >= tailleChaine ) goto ERREUR_MEMOIRE;
                resultat = (char*)realloc(resultat,(strlen(resultat)+strlen(chaine)+1)*sizeof(char));
                if ( resultat == NULL ) goto ERREUR_ALLOCATION;
                strcat(resultat,chaine);
