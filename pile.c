...
void empilerInt(TIntPile * _pile, int _val) {
    if(_pile != NULL) {
        if(_pile->data == NULL) {
            _pile->data = (int*)malloc(sizeof(int));
            _pile->size = sizeof(int);
            if(_pile->data == NULL)
                printf("Error malloc\n");
        }
        else if(_pile->size < (_pile->indexSommet + 2) * sizeof(int)) {
            _pile->size += sizeof(int);
...
void empilerVoid(TVoidPile * _pile, void * _val) {
    if(_pile != NULL) {
        if(_pile->data == NULL) {
            _pile->data = (void**)malloc(sizeof(void*));
            _pile->size = sizeof(void*);
            if(_pile->data == NULL)
                printf("Error malloc\n");
        }
        else if(_pile->size < (_pile->indexSommet + 2) * sizeof(void*)) {
            _pile->size += sizeof(void*);
            _pile->data = realloc(_pile->data, _pile->size);
        }
        _pile->indexSommet++;
