...
int analyseurLR() {
    ...
    while(!fini) {
        ...
        if(strcmp(action, "acc") == 0) {
            ...
            empilerInt(pile, atoi(TableGoto[sommetInt(pile)][indexOfChar(colonneTabAction, lexeme)]));
            lexeme = JsonSymbTab[lex(lex_data)];
        }
        else if(action == NULL) { //ERREUR
            printf("Erreur\n");
            return -1;
        }
        else {
            ...
            empilerInt(pile, atoi(TableGoto[indexOfChar(colonneTabGoto, TableRegle[nRegle][0][0])][sommetInt(pile)]));
        }
